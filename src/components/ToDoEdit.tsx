...
function ToDoEdit({editTask, task, keyValue}: Props) {
    ...
    return <>
        <form key={keyValue} onSubmit={handleSubmit(handleForm)}>
            <Stack  position={'relative'} spacing='1rem' maxWidth={600}
                   sx={{marginBottom: '0.5rem'}}>
                <TextField {...register('updatedTask')} style={{minWidth: 650, maxHeight: '50px'}} color='primary' label="Please edit task"
                            variant="filled" sx={{backgroundColor: '#eeeeee', borderRadius: '5px'}}/>
                <Button type='submit' color='secondary' style={{maxHeight: '3rem'}} sx={{position: 'absolute', bottom: '0.5rem', right: '-2.5rem'}} variant='contained'>Update</Button>
            </Stack>
        </form>
    </>
}
